...
async def register(data: RegisterData):
    ...
    if resp.user is None:
        raise HTTPException(400, "Falha no cadastro")
    return TokenResponse(access_token=resp.session.access_token if resp.session else "", refresh_token=resp.session.refresh_token if resp.session else None)
